(function() {
  ...
  game = {
    ...
    reset: function() {
      ...
      for (
        let index = this.snake.segment_indices.length - 1;
        index > -1;
        --index
      ) {
        this.world.map[this.snake.segment_indices[index]] =
          display.background_tile;
      }
    ...
    loop: function(time_stamp) {
      ...
      if (time_stamp >= game.accumulated_time + game.time_step) {
        ...
        if (game.snake.vector_x != 0 || game.snake.vector_y != 0) {
          ...
          game.snake.head_index +=
            game.snake.vector_y * game.world.columns + game.snake.vector_x; // move the snake's head index

          /* Do collision detection, testing to see if the snake's head index is
          offscreen or colliding with another snake segment. */
          if (
            game.world.map[game.snake.head_index] == display.segment || // hit a segment
            ...
            (game.snake.vector_x == -1 &&
              game.snake.head_index % game.world.columns ==
                game.world.columns - 1) || // off the left of the map
            (game.snake.vector_x == 1 &&
          ...
          if (game.snake.head_index == game.apple.index) {
            ...
            game.apple.index = Math.floor(
              Math.random() * game.world.map.length
            ); // reset the apple
            ...
            while (
              game.world.map[game.apple.index] != display.background_tile
            ) {
              game.apple.index++;
  ...
  display.buffer.canvas.height = display.buffer.canvas.width =
